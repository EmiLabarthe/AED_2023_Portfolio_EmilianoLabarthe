...
public class TGrafoDirigido implements IGrafoDirigido {
 ...
 pasadas por par�metro deben ser v�lidas.
    ...
    /**
     * Retorna un HashMap cuyas claves son el destino, y cuyos valores son el
     * TCamino mas corto desde el origen indicado como parametro, hasta el
     * destino (respectivo a la clave en el mapa)
     *
     * @param etiquetaOrigen
     * @param etiquetaDestino
     * @return HashMap cuya clave es el destino y cuyo valor es el TCamino mas
     * corto
     */
    public Map<Comparable, TCamino> dijkstra(Comparable etiquetaOrigen) {

        TVertice origen = vertices.get(etiquetaOrigen);
        Map<Comparable, TCamino> dijkstra = new HashMap();

        if (origen != null) {
            desvisitarVertices();
            Double[][] matrizA = UtilGrafos.obtenerMatrizCostos(vertices);
            // hallo indice del origen
            int i = new LinkedList<>(vertices.keySet()).indexOf(etiquetaOrigen);
            // inicializo D
            Double[] costos = new Double[vertices.size()];
            for (Comparable v : vertices.keySet()) {
                // indice en la matriz del vertice destino 
                int j = new LinkedList<>(vertices.keySet()).indexOf(v);
                costos[j] = matrizA[i][j];
                // inicializo mapa a retornar
                TCamino camino = new TCamino(origen);
                if (v.compareTo(etiquetaOrigen) != 0) {
                    camino.agregarAdyacencia(
                            new TAdyacencia(costos[j], vertices.get(v)));
                }
                dijkstra.put(v, camino);
            }

            // me aseguro de calcular el camino mas corto para todos
            for (int x = 0; x < vertices.size(); x++) {
                TVertice minVertice = null;
                double minCosto = Double.MAX_VALUE;
                for (Map.Entry<Comparable, TCamino> entry : dijkstra.entrySet()) {
                    TVertice v = vertices.get(entry.getKey());
                    TCamino camino = entry.getValue();
                    if (camino.getCostoTotal() <= minCosto && !v.getVisitado()) {
                        minVertice = v;
                        minCosto = camino.getCostoTotal();
                    }
                }
                minVertice.dijkstra(dijkstra);
            }
